...
class FakeAppServer {
  ...
  async threadUnarchive(params: { threadId: string }): Promise<unknown> {
    this.calls.push({ method: "thread/unarchive", params });
    return { ok: true };
  }
...
async function createHarness(officialIpc = createBridge()): Promise<Harness> {
  const root = mkdtempSync(join(tmpdir(), "codex-web-thread-unarchive-"));
...
describe("thread unarchive route", () => {
  it("rejects local unarchive mutations for externally owned threads", async () => {
    ...
    expect(response.json()).toMatchObject({
      error: "official-owner-action-required:thread-unarchive",
    });
