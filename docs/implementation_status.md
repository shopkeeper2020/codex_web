...
2026-06-03 续跑修复：修复 Web 新会话首条消息报 `no rollout found for thread id ...`。按 OpenAI Codex app-server README、`codex-rs/app-server` request processor 和 `app-server-protocol` v2 schema 复核后确认：新会话官方 lifecycle 是 `thread/start -> turn/start`，`thread/resume` 只用于继续已有/已持久化 thread。此前清理把所有 Web 本地 owner turn 都强制改为 `thread/resume` 成功后再 `turn/start`，导致刚 `thread/start` 的空 thread 在 rollout 尚未 materialize 时被误走 resume。现在只在“Web-owned 且 stream state 明确 `turns: []`”的首轮本地 turn 跳过 resume，直接调用官方 raw `turn/start`；已有/非空会话仍保持先 `thread/resume` 再 `turn/start`，避免分叉。发布前同时收紧了 app-shell E2E 中与当前真实运行态耦合的旧选择器/布局断言，避免回归套件因当前会话正在运行而假失败。验证：官方文档/源码复核、`pnpm --filter @codex-web/server exec vitest run src/threadActions.test.ts src/turnRoutes.test.ts src/threadCreateRoute.test.ts`（31 passed）、`pnpm typecheck`、`git diff --check`、`pnpm test`（server 167 passed，web 27 passed，protocol 40 passed）、`pnpm build`、`pnpm exec playwright test tests/e2e/app-shell.spec.ts tests/e2e/composer-runtime.spec.ts --project=desktop-chromium --project=mobile-chromium`（49 passed / 27 skipped）。生产服务已重启到 `0.0.0.0:18930`，node PID `113104`，app-server child PID `114536`；`/health`、`/api/health`、official IPC 和 sync readiness 均正常，readiness 只保留插件图标路径 warning（`interface.icon_large` 路径校验）与可选 follower handler warning。真实 HTTP 探针创建 thread `019e89a6-97ca-78a3-9831-809527cfcaf6` 并发送首条 `只回复 OK`，`turn-start` 返回 `mode: "app-server"`、turn `inProgress`，随后 detail 从 official IPC 读到完成回复 `OK`，未再出现 `no rollout found`。
...
2026-06-03 续跑修复：收敛“Web 新建会话导致 Desktop 报错”和“Web 新建会话过一会儿才出现在 Desktop 侧栏”两个相邻问题。报错根因是 Web-owned `thread-stream-state-changed` snapshot 不满足官方 Desktop live UI 的安全形状，renderer/侧栏会读取 `requests.length`、turn/item 默认字段、`turnsPagination` 等字段；现在新建 idle snapshot、首轮 pending snapshot 和协议广播归一化都会补齐 Desktop 需要的 UI 默认值。侧栏慢不是持久化失败，现场确认新 thread 已立即进入官方 `state_5.sqlite`；慢的是 Desktop recent-list 内存刷新。官方 Desktop 对外部 owner 的 stream snapshot 只 `setConversation`，不会 `ensureRecentConversationId`；Web 新建 thread 成功广播 idle snapshot 后会额外发送官方 `thread-unarchived` IPC 生命周期广播，借用 Desktop `handleThreadUnarchived -> refreshRecentConversations()` 路径刷新侧栏。清洗代码时应保留 schema-safe snapshot、idle snapshot、pending snapshot 和 `broadcastThreadUnarchived` recent refresh 这四个闭环，其他实验性 owner 猜测和重复广播逻辑可逐项用测试/手测回退。验证：`pnpm --filter @codex-web/protocol exec vitest run src/officialIpc.test.ts`、`pnpm --filter @codex-web/server exec vitest run src/threadCreateRoute.test.ts`、`pnpm --filter @codex-web/server exec vitest run src/turnRoutes.test.ts`、`pnpm --filter @codex-web/protocol typecheck`、`pnpm --filter @codex-web/protocol build`、`pnpm --filter @codex-web/server typecheck`、`pnpm --filter @codex-web/server build`。生产后端已重启到 `0.0.0.0:18930`，`/api/health` 和 official IPC 均正常。

2026-06-03 清理：按官方 app-server README、官方 `app-server-protocol` method gating/source 和本地生成 schema 复核 raw app-server 调用边界。`initialize` 握手现在只发送官方 `clientInfo.name/title/version` 与 `capabilities.experimentalApi: true`，`initialized` notification 不再携带空 `params`，出站帧省略 wire 上冗余的 `jsonrpc` 字段。Web 本地 owner 的 turn 启动改为 `thread/resume` 成功后才 `turn/start`，不再吞掉 resume 错误继续写入本地 turn；model/reasoning/collaboration follower request 直接使用官方 `thread/settings/update` 作为唯一 next-turn 状态源，删除本地 runtime settings Map 兜底。raw `turn/start` / `turn/steer` 白名单移除未使用且不在稳定 schema 中的 `additionalContext`、`responsesapiClientMetadata`。验证：`pnpm --filter @codex-web/server exec vitest run src/threadActions.test.ts src/syncCoordinator.test.ts src/turnRoutes.test.ts src/protocolCompatibility.test.ts src/threadCreateRoute.test.ts`、`pnpm --filter @codex-web/server typecheck`、`pnpm --filter @codex-web/protocol exec vitest run src/officialIpc.test.ts`、`pnpm --filter @codex-web/server build`、`pnpm --filter @codex-web/protocol build`、`pnpm --filter @codex-web/web build`、`git diff --check`。

2026-06-02 续跑修复：复查“Web 新建天气会话后 Web 闪几次、Desktop 仍红屏”。官方 app-server `thread/start` / `turn/start` 请求边界仍正常；现场 Desktop Sentry 面包屑显示红屏前的明确 renderer 错误是 `Could not find the language 'powershell'`，对应打开排障长会话时的代码高亮模块缺失，不是天气线程 raw RPC 失败。Web 闪烁原因在前端实时 reducer：`item/started` 阶段未把 `mcpToolOutput` / `webSearch` 识别为 `toolOutput`，会先渲染成 unknown，轮询到 domain detail 后再合并成折叠工具组。现在实时 reducer 与 domain 归一化对齐，工具输出在实时阶段即折叠分组；协议层也对完全相同的 Web-owned `thread-stream-state-changed` snapshot 做去重，减少 VS Code 扩展 “Received broadcast but no handler is configured” 日志风暴。验证：`pnpm --filter @codex-web/web exec vitest run src/app/appServerRealtimeReducer.test.ts`、`pnpm --filter @codex-web/web typecheck`、`pnpm --filter @codex-web/protocol exec vitest run src/officialIpc.test.ts`、`pnpm --filter @codex-web/protocol typecheck`、`pnpm --filter @codex-web/protocol build`、`pnpm --filter @codex-web/server exec vitest run src/syncCoordinator.test.ts src/officialIpcEvents.test.ts src/turnRoutes.test.ts src/threadCreateRoute.test.ts`、`pnpm --filter @codex-web/server typecheck`。

2026-06-02 续跑修复：用户再次复现后确认，Desktop 成功 `thread/resume` 新线程 `019e888a-1d72-7962-84d2-e17a2af561ba`，但同一窗口仍会恢复/渲染排障长线程 `019e8854-4ea7-7e32-a978-e65d2b88c240`；红屏前加载 `highlight-code` 后继续报 `Could not find the language 'powershell'`。根因是 Web 打开官方长线程时会把 app-server conversation snapshot 存入/广播给官方客户端，snapshot 中历史 Markdown/工具输出包含行首 ````powershell` fence，官方 Desktop 高亮器缺该语言时进入 error boundary。现在 `packages/protocol` 在所有 stream state 入缓存/出广播前递归降级不支持的 Markdown fence 语言：只把行首 `powershell` fence 改为 `text`，不改原始 rollout，不改普通正文、命令、diff 里的字符串。已手动触发当前长线程 `/api/domain/thread-detail`，并确认 `/api/official-thread-stream-state` 不再含行首 `powershell` fence。验证：`pnpm --filter @codex-web/protocol exec vitest run src/officialIpc.test.ts`、`pnpm --filter @codex-web/protocol typecheck`、`pnpm --filter @codex-web/protocol build`、`pnpm --filter @codex-web/server exec vitest run src/syncCoordinator.test.ts src/officialIpcEvents.test.ts src/turnRoutes.test.ts src/threadCreateRoute.test.ts`、`pnpm --filter @codex-web/server typecheck`、`pnpm --filter @codex-web/web exec vitest run src/app/appServerRealtimeReducer.test.ts`、`pnpm --filter @codex-web/web typecheck`。

2026-06-02 续跑修复：对比 VS Code 扩展 `26.5527.31454` 现场源码与日志，确认 Web 新建会话导致 Desktop/扩展报错的当前根因是官方 IPC client discovery shape 已变化：router 发送 `{ type: "client-discovery-request", requestId, request }`，client 必须回复 `{ type: "client-discovery-response", requestId, response: { canHandle } }`。旧 Web bridge 读取顶层 `method/version/params` 并只返回顶层 `canHandle`，导致扩展 `handleClientDiscoveryResponse` 读取 `undefined.canHandle` 后显示“未知错误”。现在 `packages/protocol` 同时兼容旧顶层与当前嵌套 discovery request，并按官方形态回复 `response.canHandle`；保留顶层字段仅作旧测试/旧 peer 兼容。验证：`pnpm --filter @codex-web/protocol exec vitest run src/officialIpc.test.ts`、`pnpm --filter @codex-web/server exec vitest run src/syncCoordinator.test.ts src/officialIpcEvents.test.ts src/turnRoutes.test.ts src/threadCreateRoute.test.ts`、`pnpm --filter @codex-web/protocol typecheck`、`pnpm --filter @codex-web/server typecheck`、`pnpm --filter @codex-web/protocol build`、`pnpm --filter @codex-web/server build`。当前 dev 服务已由 `tsx watch` 自动重启并重新连接 `\\.\pipe\codex-ipc`，`/api/health` 和 `/api/official-ipc/status` 正常。

2026-06-02 续跑修复：继续按官方 Codex app-server 手册和本地生成的 `app-server-protocol` v2 schema 排查 Web 新建会话导致 Desktop 崩溃。确认 raw `thread/start` / `turn/start` 调用边界仍符合官方要求，新的风险点在 Web-owned `thread-stream-state-changed` 广播：pending turn 里的 `userMessage` 缺少官方 `ThreadItem.userMessage.clientId` 必需字段，且后续 app-server item 广播没有统一补齐 `agentMessage.phase/memoryCitation`、`webSearch.action.queries` 等 v2 必需默认值，Desktop live renderer/侧栏 row 可能读到 `undefined.length` 后进入 error boundary。现在协议广播层会在发送给官方客户端前把 app-server Thread/Turn/ThreadItem 统一归一化为 schema-safe live snapshot；首轮 pending `userMessage.clientId` 与 `turn/start.clientUserMessageId` 对齐。验证：`pnpm --filter @codex-web/protocol exec vitest run src/officialIpc.test.ts`、`pnpm --filter @codex-web/server exec vitest run src/threadCreateRoute.test.ts src/turnRoutes.test.ts`。

2026-06-02 续跑修复：继续按官方 Codex app-server 手册、本地生成的 `app-server-protocol` v2 schema 与 Desktop/VS Code IPC 证据复查 Web 新建会话。确认仅在首轮 `turn/start` 前广播 active pending snapshot 仍会留下一个窗口：`thread/start` 已把新 thread 写入官方 app-server，但 Web 只 claim local-only owner、没有发布 `thread-stream-state-changed`，Desktop 可能先从 thread list 看到该会话却没有对应 live conversation state，打开或收到后续事件时进入 unknown conversation/error boundary。现在 `/api/domain/thread-create` 在 raw `thread/start` 成功后会立即广播一个 Desktop 可读的 idle snapshot：`status/threadRuntimeStatus: { type: "idle" }`、完整官方 `Thread` 必需字段和 `turns: []`；首条消息开始前仍会广播 active pending-turn snapshot。前端新草稿发送也改为 thread 创建成功后立即选中新会话并放置稳定 idle detail，发送完成后不再用空 turns 覆盖实时详情，减少会话区闪烁。验证：`pnpm --filter @codex-web/server exec vitest run src/threadCreateRoute.test.ts src/turnRoutes.test.ts src/accountStatusRoute.test.ts`、`pnpm --filter @codex-web/protocol exec vitest run src/officialIpc.test.ts`、`pnpm --filter @codex-web/server typecheck`、`pnpm --filter @codex-web/web typecheck`、`pnpm build`。生产服务已重启到 `0.0.0.0:18930`，进程为 `node apps/server/dist/index.js` PID `7836`，`/api/health` 通过。

2026-06-02 续跑修复：再次按官方 Codex app-server 手册和本地生成的 `app-server-protocol` v2 schema 核查 Web 新建会话。确认 `/api/domain/thread-create` 的 raw `thread/start` 只传 `cwd`、`threadSource: "user"` 是符合官方要求的；Desktop 报错根因在后续 Web-owned 首轮 `turn/start` 前没有先把完整 live stream state 广播给官方客户端，导致 Desktop 收到 `turn/started` 时仍把 conversation 视为 unknown。现在 Web 本地 owner 路径会在调用 app-server `turn/start` 前同步发布一个 Desktop 可读的 pending-turn snapshot：包含官方 `Thread` 必需字段、`threadRuntimeStatus.activeFlags`、`Turn.itemsView/error/startedAt/completedAt/durationMs`、pending `userMessage`、`diff: []`、`hookRuns: []` 和 `commandExecutionStartedAtMsById: {}`；如果刚创建的 rollout 暂时为空，会用 Web 缓存的 thread detail 生成最小官方形状，不再广播空会话。协议层 broadcast normalizer 也会把 app-server snapshot 中的 `null`/错误类型字段收敛为 Desktop 期待的数组或对象。验证：`pnpm --filter @codex-web/protocol exec vitest run src/officialIpc.test.ts`、`pnpm --filter @codex-web/server exec vitest run src/turnRoutes.test.ts src/threadCreateRoute.test.ts`、`pnpm --filter @codex-web/server exec vitest run src/syncCoordinator.test.ts src/officialIpcEvents.test.ts src/syncReadiness.test.ts`、`pnpm --filter @codex-web/server typecheck`、`pnpm --filter @codex-web/protocol build`、`pnpm --filter @codex-web/server build`。生产服务已重启到 `0.0.0.0:18930`，进程为 `node dist/index.js` PID `18776`，`/api/health` 通过。
...
2026-05-31 续跑修复：修复 Web 新建会话后官方 Desktop 侧边栏 error boundary。现场 Desktop 日志显示 `/api/domain/thread-create` 生成的新会话随后触发官方 `thread/list`，侧边栏 row 渲染报 `Cannot read properties of undefined (reading 'length')`；对应 Web 创建的 session meta 缺少官方 UI 默认带入的 `thread_source`，且 Web 还会立刻广播空 Web-owned stream snapshot。现在 Web 调用 app-server `thread/start` 会补 `threadSource: "user"`，新建会话只 claim local-only owner 供 Web 自己继续 `turn-start` fallback，不再向 Desktop/VS Code 发布空 snapshot；本地 app-server 通知和 approval 事件也会跳过 local-only owner 的 snapshot 广播。验证：`pnpm --filter @codex-web/protocol test -- src/officialIpc.test.ts`、`pnpm --filter @codex-web/protocol build`、`pnpm --filter @codex-web/server test -- src/threadCreateRoute.test.ts src/syncCoordinator.test.ts src/turnRoutes.test.ts`。
