...
2026-06-03 续跑修复：定位用户从 Web 发起的金价会话在官方 Desktop 恢复时报 `Cannot read properties of null (reading 'settings')`。该 thread 的 raw app-server `thread/read(includeTurns:true)` 正常，rollout 可读，说明不是 `thread/start` / `turn/start` 写坏；根因是 Web-owned live snapshot 归一化曾把 Default/空协作模式补成 `latestCollaborationMode: null`，而官方证据要求 Default 模式完全省略 `collaborationMode`，Desktop renderer 恢复时会读取 `.settings`。现在 `packages/protocol` 会删除 top-level `latestCollaborationMode: null`、turn `params.collaborationMode: null` 和 `{ mode: "default" }`；非默认协作模式会保留并保证 `settings` 是对象；同时把 `webSearch.openPage/findInPage` 的 `url`/`pattern` 归一为字符串，避免同一会话中观测到的 `url: null` 再进入 Desktop 渲染路径。严格测试审核额外补齐了外部 snapshot 中字符串 `userMessage.content` 的保文归一化、side conversation 本地启动 turn 的 `thread/resume` fake 覆盖，以及 E2E approval/file-preview fixture 的当前 schema/UI 断言。验证：`git diff --check`、`pnpm typecheck`、`pnpm test`（server `39 files / 165 passed`，web `7 files / 27 passed`，protocol `40 passed`，api/domain/ui 均通过）、`pnpm build`、`pnpm exec playwright test tests/e2e/ui-fidelity-baseline.spec.ts tests/e2e/message-blocks.spec.ts --project=desktop-chromium --project=mobile-chromium`（`15 passed / 3 skipped`）。生产服务已重启到 `0.0.0.0:18930`，进程为 `node dist/index.js` PID `115256`，app-server child PID `118860`，`/health`、`/api/health` 和 official IPC 均正常；readiness 仅保留远程插件缓存同步 warning 与可选 follower handler warning。

2026-06-03 续跑修复：收敛“Web 新建会话导致 Desktop 报错”和“Web 新建会话过一会儿才出现在 Desktop 侧栏”两个相邻问题。报错根因是 Web-owned `thread-stream-state-changed` snapshot 不满足官方 Desktop live UI 的安全形状，renderer/侧栏会读取 `requests.length`、turn/item 默认字段、`turnsPagination` 等字段；现在新建 idle snapshot、首轮 pending snapshot 和协议广播归一化都会补齐 Desktop 需要的 UI 默认值。侧栏慢不是持久化失败，现场确认新 thread 已立即进入官方 `state_5.sqlite`；慢的是 Desktop recent-list 内存刷新。官方 Desktop 对外部 owner 的 stream snapshot 只 `setConversation`，不会 `ensureRecentConversationId`；Web 新建 thread 成功广播 idle snapshot 后会额外发送官方 `thread-unarchived` IPC 生命周期广播，借用 Desktop `handleThreadUnarchived -> refreshRecentConversations()` 路径刷新侧栏。清洗代码时应保留 schema-safe snapshot、idle snapshot、pending snapshot 和 `broadcastThreadUnarchived` recent refresh 这四个闭环，其他实验性 owner 猜测和重复广播逻辑可逐项用测试/手测回退。验证：`pnpm --filter @codex-web/protocol exec vitest run src/officialIpc.test.ts`、`pnpm --filter @codex-web/server exec vitest run src/threadStartRoute.test.ts`、`pnpm --filter @codex-web/server exec vitest run src/turnRoutes.test.ts`、`pnpm --filter @codex-web/protocol typecheck`、`pnpm --filter @codex-web/protocol build`、`pnpm --filter @codex-web/server typecheck`、`pnpm --filter @codex-web/server build`。生产后端已重启到 `0.0.0.0:18930`，`/api/health` 和 official IPC 均正常。

2026-06-03 清理：按官方 app-server README、官方 `app-server-protocol` method gating/source 和本地生成 schema 复核 raw app-server 调用边界。`initialize` 握手现在只发送官方 `clientInfo.name/title/version` 与 `capabilities.experimentalApi: true`，`initialized` notification 不再携带空 `params`，出站帧省略 wire 上冗余的 `jsonrpc` 字段。Web 本地 owner 的 turn 启动改为 `thread/resume` 成功后才 `turn/start`，不再吞掉 resume 错误继续写入本地 turn；model/reasoning/collaboration follower request 直接使用官方 `thread/settings/update` 作为唯一 next-turn 状态源，删除本地 runtime settings Map 兜底。raw `turn/start` / `turn/steer` 白名单移除未使用且不在稳定 schema 中的 `additionalContext`、`responsesapiClientMetadata`。验证：`pnpm --filter @codex-web/server exec vitest run src/threadActions.test.ts src/syncCoordinator.test.ts src/turnRoutes.test.ts src/protocolCompatibility.test.ts src/threadStartRoute.test.ts`、`pnpm --filter @codex-web/server typecheck`、`pnpm --filter @codex-web/protocol exec vitest run src/officialIpc.test.ts`、`pnpm --filter @codex-web/server build`、`pnpm --filter @codex-web/protocol build`、`pnpm --filter @codex-web/web build`、`git diff --check`。

2026-06-02 续跑修复：复查“Web 新建天气会话后 Web 闪几次、Desktop 仍红屏”。官方 app-server `thread/start` / `turn/start` 请求边界仍正常；现场 Desktop Sentry 面包屑显示红屏前的明确 renderer 错误是 `Could not find the language 'powershell'`，对应打开排障长会话时的代码高亮模块缺失，不是天气线程 raw RPC 失败。Web 闪烁原因在前端实时 reducer：`item/started` 阶段未把 `mcpToolOutput` / `webSearch` 识别为 `toolOutput`，会先渲染成 unknown，轮询到 domain detail 后再合并成折叠工具组。现在实时 reducer 与 domain 归一化对齐，工具输出在实时阶段即折叠分组；协议层也对完全相同的 Web-owned `thread-stream-state-changed` snapshot 做去重，减少 VS Code 扩展 “Received broadcast but no handler is configured” 日志风暴。验证：`pnpm --filter @codex-web/web exec vitest run src/app/appServerRealtimeReducer.test.ts`、`pnpm --filter @codex-web/web typecheck`、`pnpm --filter @codex-web/protocol exec vitest run src/officialIpc.test.ts`、`pnpm --filter @codex-web/protocol typecheck`、`pnpm --filter @codex-web/protocol build`、`pnpm --filter @codex-web/server exec vitest run src/syncCoordinator.test.ts src/officialIpcEvents.test.ts src/turnRoutes.test.ts src/threadStartRoute.test.ts`、`pnpm --filter @codex-web/server typecheck`。

2026-06-02 续跑修复：用户再次复现后确认，Desktop 成功 `thread/resume` 新线程 `019e888a-1d72-7962-84d2-e17a2af561ba`，但同一窗口仍会恢复/渲染排障长线程 `019e8854-4ea7-7e32-a978-e65d2b88c240`；红屏前加载 `highlight-code` 后继续报 `Could not find the language 'powershell'`。根因是 Web 打开官方长线程时会把 app-server conversation snapshot 存入/广播给官方客户端，snapshot 中历史 Markdown/工具输出包含行首 ````powershell` fence，官方 Desktop 高亮器缺该语言时进入 error boundary。现在 `packages/protocol` 在所有 stream state 入缓存/出广播前递归降级不支持的 Markdown fence 语言：只把行首 `powershell` fence 改为 `text`，不改原始 rollout，不改普通正文、命令、diff 里的字符串。已手动触发当前长线程 `/api/domain/thread/read`，并确认 `/api/official-thread-stream-state` 不再含行首 `powershell` fence。验证：`pnpm --filter @codex-web/protocol exec vitest run src/officialIpc.test.ts`、`pnpm --filter @codex-web/protocol typecheck`、`pnpm --filter @codex-web/protocol build`、`pnpm --filter @codex-web/server exec vitest run src/syncCoordinator.test.ts src/officialIpcEvents.test.ts src/turnRoutes.test.ts src/threadStartRoute.test.ts`、`pnpm --filter @codex-web/server typecheck`、`pnpm --filter @codex-web/web exec vitest run src/app/appServerRealtimeReducer.test.ts`、`pnpm --filter @codex-web/web typecheck`。

2026-06-02 续跑修复：对比 VS Code 扩展 `26.5527.31454` 现场源码与日志，确认 Web 新建会话导致 Desktop/扩展报错的当前根因是官方 IPC client discovery shape 已变化：router 发送 `{ type: "client-discovery-request", requestId, request }`，client 必须回复 `{ type: "client-discovery-response", requestId, response: { canHandle } }`。旧 Web bridge 读取顶层 `method/version/params` 并只返回顶层 `canHandle`，导致扩展 `handleClientDiscoveryResponse` 读取 `undefined.canHandle` 后显示“未知错误”。现在 `packages/protocol` 同时兼容旧顶层与当前嵌套 discovery request，并按官方形态回复 `response.canHandle`；保留顶层字段仅作旧测试/旧 peer 兼容。验证：`pnpm --filter @codex-web/protocol exec vitest run src/officialIpc.test.ts`、`pnpm --filter @codex-web/server exec vitest run src/syncCoordinator.test.ts src/officialIpcEvents.test.ts src/turnRoutes.test.ts src/threadStartRoute.test.ts`、`pnpm --filter @codex-web/protocol typecheck`、`pnpm --filter @codex-web/server typecheck`、`pnpm --filter @codex-web/protocol build`、`pnpm --filter @codex-web/server build`。当前 dev 服务已由 `tsx watch` 自动重启并重新连接 `\\.\pipe\codex-ipc`，`/api/health` 和 `/api/official-ipc/status` 正常。

2026-06-02 续跑修复：继续按官方 Codex app-server 手册和本地生成的 `app-server-protocol` v2 schema 排查 Web 新建会话导致 Desktop 崩溃。确认 raw `thread/start` / `turn/start` 调用边界仍符合官方要求，新的风险点在 Web-owned `thread-stream-state-changed` 广播：pending turn 里的 `userMessage` 缺少官方 `ThreadItem.userMessage.clientId` 必需字段，且后续 app-server item 广播没有统一补齐 `agentMessage.phase/memoryCitation`、`webSearch.action.queries` 等 v2 必需默认值，Desktop live renderer/侧栏 row 可能读到 `undefined.length` 后进入 error boundary。现在协议广播层会在发送给官方客户端前把 app-server Thread/Turn/ThreadItem 统一归一化为 schema-safe live snapshot；首轮 pending `userMessage.clientId` 与 `turn/start.clientUserMessageId` 对齐。验证：`pnpm --filter @codex-web/protocol exec vitest run src/officialIpc.test.ts`、`pnpm --filter @codex-web/server exec vitest run src/threadStartRoute.test.ts src/turnRoutes.test.ts`。

2026-06-02 续跑修复：继续按官方 Codex app-server 手册、本地生成的 `app-server-protocol` v2 schema 与 Desktop/VS Code IPC 证据复查 Web 新建会话。确认仅在首轮 `turn/start` 前广播 active pending snapshot 仍会留下一个窗口：`thread/start` 已把新 thread 写入官方 app-server，但 Web 只 claim local-only owner、没有发布 `thread-stream-state-changed`，Desktop 可能先从 thread list 看到该会话却没有对应 live conversation state，打开或收到后续事件时进入 unknown conversation/error boundary。现在 `/api/domain/thread/start` 在 raw `thread/start` 成功后会立即广播一个 Desktop 可读的 idle snapshot：`status/threadRuntimeStatus: { type: "idle" }`、完整官方 `Thread` 必需字段和 `turns: []`；首条消息开始前仍会广播 active pending-turn snapshot。前端新草稿发送也改为 thread 创建成功后立即选中新会话并放置稳定 idle detail，发送完成后不再用空 turns 覆盖实时详情，减少会话区闪烁。验证：`pnpm --filter @codex-web/server exec vitest run src/threadStartRoute.test.ts src/turnRoutes.test.ts src/accountStatusRoute.test.ts`、`pnpm --filter @codex-web/protocol exec vitest run src/officialIpc.test.ts`、`pnpm --filter @codex-web/server typecheck`、`pnpm --filter @codex-web/web typecheck`、`pnpm build`。生产服务已重启到 `0.0.0.0:18930`，进程为 `node apps/server/dist/index.js` PID `7836`，`/api/health` 通过。

2026-06-02 续跑修复：再次按官方 Codex app-server 手册和本地生成的 `app-server-protocol` v2 schema 核查 Web 新建会话。确认 `/api/domain/thread/start` 的 raw `thread/start` 只传 `cwd`、`threadSource: "user"` 是符合官方要求的；Desktop 报错根因在后续 Web-owned 首轮 `turn/start` 前没有先把完整 live stream state 广播给官方客户端，导致 Desktop 收到 `turn/started` 时仍把 conversation 视为 unknown。现在 Web 本地 owner 路径会在调用 app-server `turn/start` 前同步发布一个 Desktop 可读的 pending-turn snapshot：包含官方 `Thread` 必需字段、`threadRuntimeStatus.activeFlags`、`Turn.itemsView/error/startedAt/completedAt/durationMs`、pending `userMessage`、`diff: []`、`hookRuns: []` 和 `commandExecutionStartedAtMsById: {}`；如果刚创建的 rollout 暂时为空，会用 Web 缓存的 thread detail 生成最小官方形状，不再广播空会话。协议层 broadcast normalizer 也会把 app-server snapshot 中的 `null`/错误类型字段收敛为 Desktop 期待的数组或对象。验证：`pnpm --filter @codex-web/protocol exec vitest run src/officialIpc.test.ts`、`pnpm --filter @codex-web/server exec vitest run src/turnRoutes.test.ts src/threadStartRoute.test.ts`、`pnpm --filter @codex-web/server exec vitest run src/syncCoordinator.test.ts src/officialIpcEvents.test.ts src/syncReadiness.test.ts`、`pnpm --filter @codex-web/server typecheck`、`pnpm --filter @codex-web/protocol build`、`pnpm --filter @codex-web/server build`。生产服务已重启到 `0.0.0.0:18930`，进程为 `node dist/index.js` PID `18776`，`/api/health` 通过。

2026-06-02 续跑修复：修复图片附件发送时用户正文被追加 `<image>` 的问题。按官方 `app-server-protocol` 的 `UserInput` schema 复查后确认，`localImage` 是独立 input 分支，不需要额外 `{ type: "text", text: "<image>" }` 占位；Web 后端现在只发送用户真实文本和 `{ type: "localImage", path }`，`restoreMessage` 仍保留图片缩略图恢复信息。`<image>` 只保留在 domain legacy 读取层作为旧缓存过滤目标。验证：`pnpm --filter @codex-web/server exec vitest run src/turnRoutes.test.ts`、`pnpm --filter @codex-web/server typecheck`、`pnpm --filter @codex-web/server build`。生产服务已重启到 `0.0.0.0:18930`。

2026-06-02 续跑修复：按官方 app-server README/source/schema 重新收口 Web 对 raw app-server 的调用边界。`turn/start` / `turn/steer` 现在会生成并传递官方 `clientUserMessageId`，直接调用 raw app-server 前会白名单化参数，剥离只属于 UI/IPC 恢复层的 `attachments`、`restoreMessage`；图片附件改为官方 `{ type: "localImage", path }` input，避免向 app-server 发送大段 data URL。Web-owned 线程收到 Desktop/扩展的模型、推理强度或协作模式变更时，调用官方 `thread/settings/update`，以 app-server loaded thread settings 作为唯一 next-turn 状态源。审批系统补齐官方 `item/permissions/requestApproval`，批准时返回 `permissions` 子集并支持 `scope: "session"`。验证：`pnpm --filter @codex-web/server exec vitest run src/threadActions.test.ts src/approvals.test.ts src/syncCoordinator.test.ts src/turnRoutes.test.ts`、`pnpm --filter @codex-web/api exec vitest run src/index.test.ts`、`pnpm --filter @codex-web/protocol exec vitest run src/officialIpc.test.ts`、`pnpm --filter @codex-web/api build`、`pnpm --filter @codex-web/server typecheck`、`pnpm --filter @codex-web/web typecheck`。

2026-06-02 续跑修复：按官方 app-server `thread/start` / `thread/started` 形状校正 Web 新建会话链路。Web 新建 thread 现在只向 raw app-server RPC 发送 `cwd` 和 `threadSource: "user"`，不再把 Desktop 内部 `start-conversation` 包装层的 `workspaceRoots` 混入 app-server 参数；Web-owned app-server 快照在广播给 Desktop/VS Code 前会转成 Desktop stream snapshot 形状，补齐 `threadRuntimeStatus.activeFlags`、turn `params`、`diff: []`、`commandExecutionStartedAtMsById`、`hookRuns` 和时间戳字段，避免官方 Desktop 渲染新会话/本地任务行时读取到 `undefined.length`。本地 app-server 非 delta 生命周期通知也会推动一次 Web-owned snapshot，减少 Desktop 打开新会话时只看到半成形状态的窗口期。验证：`pnpm --filter @codex-web/protocol exec vitest run src/officialIpc.test.ts`、`pnpm --filter @codex-web/server exec vitest run src/threadStartRoute.test.ts src/turnRoutes.test.ts src/officialIpcEvents.test.ts`、`pnpm --filter @codex-web/server typecheck`、`pnpm build`。

2026-06-01 续跑修复：修复 Web 在已空闲旧会话中发送消息时偶发 `official-owner-unavailable` 的问题。根因是某些 thread 保留了 Desktop/VS Code 或重启前 Web 的旧 owner clientId，`thread-follower-start-turn` 定向和 discovery 都失败后，后端此前只要看到官方 stream cache 就拒绝本地执行；现在仅对 `turn/start` 放宽：先读 app-server 完整 thread 快照，确认会话不是 active，必要时退休旧外部 owner cache，再 claim local-only Web owner 并用 app-server 启动这一轮。active turn 的 steer/interrupt/compact 仍保持拒绝本地 fallback，避免真实运行中分叉。验证：`pnpm --filter @codex-web/server test -- src/turnRoutes.test.ts`、`pnpm --filter @codex-web/server typecheck`、`pnpm --filter @codex-web/server test -- src/threadDetailRoute.test.ts src/officialIpcEvents.test.ts src/syncReadiness.test.ts` 均通过。
...
2026-05-31 续跑修复：修复 Web 新建会话后官方 Desktop 侧边栏 error boundary。现场 Desktop 日志显示 `/api/domain/thread/start` 生成的新会话随后触发官方 `thread/list`，侧边栏 row 渲染报 `Cannot read properties of undefined (reading 'length')`；对应 Web 创建的 session meta 缺少官方 UI 默认带入的 `thread_source`，且 Web 还会立刻广播空 Web-owned stream snapshot。现在 Web 调用 app-server `thread/start` 会补 `threadSource: "user"`，新建会话只 claim local-only owner 供 Web 自己继续 `turn/start` fallback，不再向 Desktop/VS Code 发布空 snapshot；本地 app-server 通知和 approval 事件也会跳过 local-only owner 的 snapshot 广播。验证：`pnpm --filter @codex-web/protocol test -- src/officialIpc.test.ts`、`pnpm --filter @codex-web/protocol build`、`pnpm --filter @codex-web/server test -- src/threadStartRoute.test.ts src/syncCoordinator.test.ts src/turnRoutes.test.ts`。

2026-05-31 续跑修复：把 Composer 上方的“追求目标”从静态进度展示改为真实 Desktop thread goal 状态。后端新增 `thread/goal/get`、`thread/goal/set`、`thread/goal/clear` app-server bridge 与 `/api/domain/thread/goal/set|thread/goal/clear` 路由，`/api/domain/thread/read` 会返回 `goal`，且 goal 读取有短超时保护，不会拖住主会话详情加载。前端 Composer 顶部目标条现在只在 thread 有真实 goal 时展示，并提供“编辑目标、暂停/恢复目标、清除目标、显示/隐藏完整目标”四个真实动作；它和右侧 pinned summary 的 plan/progress 步骤分离，也不等同于 Composer 底栏的可选 Plan/目标模式。验证：`pnpm --filter @codex-web/domain test -- src/index.test.ts`、`pnpm --filter @codex-web/api test -- src/index.test.ts`、`pnpm --filter @codex-web/server test -- src/threadGoalRoute.test.ts src/threadDetailRoute.test.ts`、`pnpm --filter @codex-web/server typecheck`、`pnpm --filter @codex-web/server build`、`pnpm --filter @codex-web/web typecheck`、`pnpm --filter @codex-web/web build`、`pnpm test:e2e -- tests/e2e/app-shell.spec.ts --project=desktop-chromium --project=mobile-chromium` 均通过；生产后端已重启，当前主 thread detail 可读到 active goal。
...
2026-05-31 续跑修复：侧边聊天从“只能读官方 sideConversation”推进到“已有官方侧聊可从 Web 发送”。右侧栏侧边聊天输入框现在会把消息发送到对应 `sideConversation.id` 的 `/api/domain/turn/start`，复用官方 follower 路径并刷新父会话详情；不会调用 `/api/domain/thread/start`，避免 Web 生成私有分叉。app-shell 回归覆盖两个真实侧边聊天标签：`ui和ux有什么区别？` 和 `侧边聊天 2` 都会以各自 side conversation id 发起 turn/start，且 `thread/start` 调用次数保持为 0。新建全新的侧边聊天/fork 协议仍待确认，当前只对已由 Desktop/官方同步出的侧聊开放发送。
...
2026-05-30 续跑状态：修复 Desktop active turn 流式输出在 Web 侧变成“有进行中状态但没有尾部内容”的问题。现场确认 `/api/official-ipc/status` 已 connected，且 `/api/official-thread-stream-state` 能读到 `isInProgress=true`、`threadRuntimeStatus.type=active`，但官方 snapshot 中最后一个 active turn 的 `items=[]`，旧 `/api/domain/thread/read` 因为历史 turns 非空而直接返回 `official-ipc`，没有触发 app-server 只读回读，导致 Web 看不到 live tail；同时 thread list 只信 app-server list，列表行也可能显示 `inProgress=false`。现在 server 会识别 external-owned 官方详情里的“空 active turn”，改走 app-server `thread/read` 只读返回并 hydrate official stream cache，不写 SQLite detail cache、不广播、不声明 Web owner；thread list 也会用 official stream cache 覆盖当前页的 owner / inProgress / updatedAt。前端在当前会话处于 active 时增加 1.5 秒轻量详情轮询，作为 WebSocket 漏事件后的兜底。
...
- 项目列表、文件浏览与工作区状态：官方 thread/list 推导出的项目仍是主来源；Web 额外支持项目收藏路径，保存到 `data/config.local.json` 的 `projects.favorites`，并通过 `/api/projects/favorites` 与官方项目列表合并展示。添加项目时后端会同步追加 Desktop `~/.codex/.codex-global-state.json` 的 `electron-saved-workspace-roots`，并保留 `active-workspace-roots` 不变；若 Desktop 已持有旧内存状态，刷新或重启 Desktop 后可收敛。侧边栏可按项目过滤会话，新建会话会优先使用当前选中项目 cwd；Settings 可查看、添加和移除项目收藏。会话主区已加入只读项目文件浏览，后端 `/api/files/list` 只允许列出官方项目、Web 收藏项目或默认项目根目录内的目录项；右侧运行栏使用 `/api/workspace/status` 读取同一批允许项目根内的 Git/GitHub CLI 状态，避免前端硬编码示例分支或提交；`apps/server/src/fileBrowser.test.ts` 覆盖目录排序、limit、拒绝 `..` 逃逸和拒绝文件路径浏览，`apps/server/src/workspaceStatusRoute.test.ts` 覆盖工作区状态路由的允许根与拒绝根。
...
GET  /api/domain/thread/list?limit=30
GET  /api/domain/thread/read?threadId=<thread-id>
POST /api/domain/thread/start
...
- Web 新建 thread 前必须已有官方 IPC `clientId`，否则后端返回 `official-ipc-owner-not-ready`；新建成功后还会确认 local-only Web owner 已建立，再把 thread 写入 Web 投影缓存。`thread/start` raw RPC 必须按官方 app-server 形状携带 `threadSource: "user"`；runtime workspace roots 由 app-server 根据 `cwd` 建立，不能把 Desktop 内部 `start-conversation` 包装层的 `workspaceRoots` 直接混入 raw app-server 参数。
...
- Web 的 rename 在明确 external-owned 时已按官方 Desktop 路径走 app-server `thread/name/set`，只读 hydrate official cache 且不声明 Web owner；archive/unarchive 仍会返回 `official-owner-action-required:*`，暂不走本地 app-server mutation。
...
- `/api/domain/thread/read` 优先读取官方 IPC 实时缓存；如果官方缓存存在但 normalizer 不能得到有效 turns，会记录 `official-thread/read-empty-fallback` 并尝试 app-server `thread/read`，避免 UI 被半截官方缓存压成空白。

## SQLite/Drizzle 本地状态

SQLite 只保存 Web 自己拥有的本地状态，不保存 app-server / 官方 IPC 可以重新读取的会话列表、会话详情或完整 `conversationState`。启动时会清理历史派生缓存表并压缩存储，避免形成第二套官方数据影子后端。
...
- `pnpm typecheck`：通过。
- `pnpm test`：通过，protocol 侧 `18 passed`，api 侧 `21 passed`，server 侧 `22 files / 81 passed`，web 侧 `5 files / 17 passed`，当前单元测试合计覆盖 protocol、domain、api、server、web、ui；API 契约测试包含 health、settings/favorites/file browser/diagnostics/cache/account/runtime/skills/attachments/attachments list+upload envelope/approvals/thread action/protocol compatibility envelope、sync readiness envelope、known realtime event union、connected server epoch、raw turn/start attachment 拒绝、concrete message item discriminated union、follower method capability matrix 和 registered request handler status，server 侧包含 protocol compatibility、sync readiness、sync coordinator、Web-owned runtime owner-state、runtime owner-state handoff 清理、approval snapshot scheduling、unknown-owner 本地 snapshot 不广播、debounce/handoff/release 后延迟 snapshot 不广播、thread/start owner-ready guard、thread/start owner 建立失败不写缓存、external-owned thread action guard、rename detail refresh 期间 owner 丢失不重广播、external-owned empty detail readonly hydrate、thread list cursor passthrough、sync doctor CLI 诊断模式只读、发送失败不继续伪轮询和 report path 解析、官方归档/恢复 IPC notification 转 realtime bus event、WebSocket realtime route schema-valid connected/server epoch 事件、诊断导出、approval requested/resolved round-trip、附件清理、发送后附件 thread 关联回归、Web-owned thread/start owner、thread rename/archive/unarchive owner guard、thread archive owner/cache release、turn HTTP 路由层的官方 owner 失败不分叉保护、generic follower request failure 分类、raw attachment 不转发保护，以及 runtime options/Skills/collaborationMode 传入 official follower 的参数贯通和脱敏诊断摘要；protocol 侧还覆盖官方 `thread-archived` / `thread-unarchived` 广播的被动收敛通知、follower start/steer/interrupt framed IPC、stale owner discovery retry 和错误诊断；web 侧包含 realtime cacheVersion、后端 websocket instance 变化后清空旧 cacheVersion、官方归档/恢复 unversioned event 接受、thread detail 请求排序、owner/IPC 错误提示映射、thread list page 合并和长会话列表 virtual window 计算。
- `pnpm build`：通过。
...
Invoke-RestMethod -Uri http://127.0.0.1:18930/api/diagnostics/export | ConvertTo-Json -Depth 12
Invoke-RestMethod -Uri "http://127.0.0.1:18930/api/domain/thread/list?limit=5&archived=true" | ConvertTo-Json -Depth 8
```
...
- official-owned rename 已确认官方 Desktop 走 app-server `thread/name/set`，Web 采用同一路径并只读 hydrate official cache；archive/unarchive 继续采用保守策略：当前勘察未找到安全的 `thread-follower-archive/unarchive` 官方路径，Web 不对 external-owned thread 做本地归档/恢复 mutation，避免分叉。`thread-follower-edit-last-user-turn` 官方实现依赖 `thread/rollback` + `turn/start`，但 rollback 不恢复本地文件变更且需要精确重建附件/审批/沙箱参数，继续标记为 risky，不在 MVP 暴露。
