...
当用户要求“部署”“重新部署”“重启”或“部署重启”时，默认同时处理两个端口：

- `18930`：后端/API/生产静态页面服务。
- `18931`：前端 Vite 开发服务器。

执行完成后必须同时检查 `18930` 和 `18931` 都在监听，并分别验证 `http://127.0.0.1:18930/health` 与 `http://127.0.0.1:18931/` 可访问；不要只重启其中一个端口。
...
- 官方 app-server `thread/search` 返回的普通会话。
- 搜索结果展示官方返回的命中片段 `snippet`。

侧边栏不再保留本地过滤输入框；项目列表仍只负责项目筛选，搜索按钮进入官方会话搜索弹窗。
