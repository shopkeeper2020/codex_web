...
| P2     | edit-last-user-turn        | 已接入官方 owner-aware 路径：official-owned thread 通过 `thread-follower-edit-last-user-turn` 转发 `conversationId + turnId + message` 给 owner；Web-owned thread 从原始 turn params 继承上下文，只替换正文后执行官方 `thread/rollback(numTurns: 1)` + `turn/start`；前端只允许最后一条已完成的普通用户消息在原气泡内编辑，`steeringUserMessage` 不显示时间/复制/编辑控件。rollback 仍不恢复本地文件变更，因此保留为高风险交互边界                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | 已有 schema、protocol、server route、local owner handler 和消息 UI 单测覆盖；仍需 Desktop/VS Code/Web 三端人工验收 rollback 后文件状态提示是否足够 |
