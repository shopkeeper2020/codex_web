...
- `apps/web/src/app/threadListPages.ts` 负责普通/归档会话分页合并与去重；前端列表通过 `/api/domain/thread/list?cursor=...` 继续读取 app-server cursor page，不直接读取官方持久化文件。
...
- 对 external-owned 的空 official snapshot，`/api/domain/thread/read` 可以临时读取 app-server 补足展示数据，但不得写入 `thread_details` cache。
